...
def _resolve_pdf_page_workers(page_count: int, max_workers: int | None) -> int:
    ...
    if page_count < _PAGE_PARALLEL_MIN_PAGES:
        return 1
    return min(extraction_cpu_workers(), page_count, _PAGE_PARALLEL_MAX_WORKERS)
...
def extract_pdf_operation_details(
    pdf_path: str,
    *,
    max_workers: int | None = None,
) -> list[ExtractedOperationDetail]:
