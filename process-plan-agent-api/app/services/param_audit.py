...
该模块负责样本/规则/人工答案的聚合与问题状态计算，
不依赖 FastAPI，也不拥有外层事务。
"""
...
from app.models.models import Document, ParamAuditAnswer
...
from app.services.llm_service import call_llm, get_prompt_template, parse_json_from_llm
...
def build_param_operation_reviews(
    factor_fields: list[FactorFieldOut],
    compiled_rules: list[dict[str, object]],
    sample_pairs: list[dict[str, object]],
    superset_stages: list[ParamJsonStageOut],
    ...
    return _build_param_operation_reviews(
        factor_fields,
        compiled_rules,
        sample_pairs,
        superset_stages,
...
async def load_param_audit_answer_map(
    db: AsyncSession,
    project_id: int,
) -> dict[str, dict[str, ParamAuditAnswer]]:
    return await _load_param_audit_answer_map(db, project_id)


def build_param_pending_questions_from_reviews(
    operation_reviews: list[ParamOperationReviewOut],
) -> list[ParamConfirmQuestionOut]:
    return _build_param_pending_questions_from_reviews(operation_reviews)


def build_param_audit_overview(
    operation_reviews: list[ParamOperationReviewOut],
    sample_pair_count: int,
) -> ParamAuditOverviewOut:
    return _build_param_audit_overview(operation_reviews, sample_pair_count)


__all__ = [
    "build_param_audit_overview",
    "build_param_operation_reviews",
    "build_param_pending_questions_from_reviews",
    "load_param_audit_answer_map",
]
