...
async def update_mapping(
    ...
    if request.target_factor_key is not None:
        mapping.target_factor_key = request.target_factor_key
        mapping.target_factor_name, mapping.target_factor_category = _catalog_by_key()[request.target_factor_key]
    if request.target_factor_name is not None and mapping.mapping_mode == "manual_factor":
        mapping.target_factor_name = request.target_factor_name.strip()
    if request.target_factor_category is not None and mapping.mapping_mode == "manual_factor":
        mapping.target_factor_category = request.target_factor_category.strip() or "manual_override"
    if request.status is not None:
        mapping.status = request.status
    mapping.updated_by = request.actor
    mapping.revision += 1
    await db.flush()
