...
| 验证项 | 实际命令 | 结果 |
| --- | --- | --- |
| 后端全量测试 | 在 `process-plan-agent-api/` 使用仓库自带 Python 3.13.5 执行 `..\\.runtime\\python\\python.exe -m pytest -q` | `385 passed, 1 skipped, 1 warning`，耗时 `34.35s` |
...
4. 后端 pytest 的唯一警告来自 FastAPI/Starlette TestClient 对 httpx 的迁移弃用提示；当前不影响 385 个通过用例，但应由 R-010 给出升级结论。
