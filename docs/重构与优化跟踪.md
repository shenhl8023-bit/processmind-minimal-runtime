...
| R-002 | P0 | 参数问答策略的单一来源与 Docker 一致性 | 未开始 | 无 | 阶段 0 |
...
- [ ] 本地后端、Docker API 和离线包读取同一策略内容及版本。
- [ ] 前端和后端对代表性工序名称得到相同的工序族和优先级顺序。
- [ ] 正式运行环境中配置缺失或 JSON 无效时启动失败并给出明确错误。
- [ ] 测试环境可以显式注入测试配置，不依赖前端源码目录。
- [ ] 删除旧文件后不存在隐式兼容读取路径。
