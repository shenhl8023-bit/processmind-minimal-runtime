...
> **文档状态：** 当前代码基线已复核，R-003 已验证完成
>
> **复核日期：** 2026-08-07
>
> **代码基线：** `1fd2863`（`修复：统一派生路线写入事务与工作流版本`）
>
...
R-003 的设计和实施计划已随本次实现复核并保持一致；其余未跟踪文档不代表已编码或已验收。
...
| 发布与执行服务 | 发布准备和 V2 执行已分别位于 `rule_packages/publishing.py`、`rule_packages/execution.py`，路由保留 HTTP 映射、事务和结果持久化。 |
...
| 验证项 | 实际命令 | 结果 |
| --- | --- | --- |
| 后端全量测试 | 在 `process-plan-agent-api/` 执行 `..\.runtime\python\python.exe -m pytest tests -q` | `313 passed, 1 skipped, 1 warning`，耗时 `24.61s` |
...
1. 本机便携 Python 为 `3.13.5`；Dockerfile 使用 `python:3.11-slim`，本次未在 Python 3.11 上运行测试。
...
4. 后端 pytest 的唯一警告来自 FastAPI/Starlette TestClient 对 httpx 的迁移弃用提示；当前不影响 313 个通过用例，但应由 R-010 给出升级结论。
...
| R-004 | P1 | 部分完成 | 发布与 V2 执行已拆出；`generate.py` 仍同时承载问答、编排和持久化 | 再拆分参数审核和路线生成应用服务 |
...
- 规则包响应构建、参数策略、问答流和输入模式已有 `generate_route_result_builder.py`、`param_question_strategy.py`、`param_question_flow.py` 等服务模块。
- 新前端生成请求会携带可选规则包指纹；规则包替换时服务端返回 `409`，而非静默跨版本执行。

#### 剩余问题

`routers/generate.py` 仍有 `1073` 行，既包含样本统计、参数问题生成、LLM 增强、V1/V2 路径选择、异常映射、`GeneratedRoute` 持久化和响应组装。当前函数注释仍写有“无规则包时回退第二步”，但实际代码在无有效包时返回 `409`；这项注释需要与当前发布边界同步。

#### 下一步

1. 将参数审核聚合移动到应用服务，直接测试稳定、待确认和数据问题分支。
2. 将 V1/V2 选择及领域结果放到路线生成服务，路由只保留锁、HTTP 映射和持久化。
3. 保持“无有效已发布包返回 409”的当前契约，并同步修正路由注释和相关说明。
...
R-003 已完成，后续状态接口及页面拆分不应绕过其写入原子性和版本冲突边界。R-004 和 R-005 可以在 R-006 的状态契约定稿后并行推进；R-010 负责把已完成条目的回归证据固化为可重复门禁。
...
| 2026-08-07 | 当前基线复核 | 更新基线至 `b126bcb` 加未提交 R-003 修复；后端 `313 passed, 1 skipped, 1 warning`，前端 `125 passed`，生产构建成功；Docker、Python 3.11、静态检查和 CI 未验证。 |
